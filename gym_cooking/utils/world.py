...
class World:
    ...
    def get_lower_bound_between_helper(self, subtask, agent_locs, A_loc, B_loc):
        ...
        for A_na, B_na in product(A_possible_na, B_possible_na):
            if len(agent_locs) == 1:
                ...
                if isinstance(subtask, recipe.Chop) or isinstance(subtask, recipe.Deliver) or isinstance(subtask, recipe.Cook):
    ...
    def get_dynamic_objects(self):
        ...
        for key in sorted(self.objects.keys()):
            if key != "Counter" and key != "Floor" and "Supply" not in key and key != "Delivery" and key != "Cutboard" and key != "Stove":
